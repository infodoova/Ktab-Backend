package com.doova.ktab.service.elevenlabs;

import com.doova.ktab.config.elevenlabs.ElevenLabsProperties;
...
@Service
@Slf4j
public class ElevenLabsTimestampTtsService {

    private final WebClient webClient;
    private final ElevenLabsProperties props;

    public ElevenLabsTimestampTtsService(WebClient.Builder webClientBuilder, ElevenLabsProperties props) {

        ExchangeStrategies strategies = ExchangeStrategies.builder().codecs(codecs -> codecs.defaultCodecs().maxInMemorySize(20 * 1024 * 1024)).build();


        this.props = props;
        this.webClient = webClientBuilder.baseUrl(firstNonBlank(props.getBaseUrl(), "https://api.elevenlabs.io")).exchangeStrategies(strategies).defaultHeader("xi-api-key", props.getApiKey()).build();
    }
    ...
    private Mono<TtsStreamChunk> streamWithTimestampsModel(String text, String voiceId, String prevText, String nextText, String modelId, List<String> previousRequestIds, Consumer<String> onRequestId) {
        ...
        return webClient.post().uri(uri -> uri.path("/v1/text-to-speech/{voiceId}/with-timestamps").queryParam("output_format", firstNonBlank(props.getOutputFormat(), "mp3_44100_128")).build(voiceId)).contentType(MediaType.APPLICATION_JSON).accept(MediaType.APPLICATION_JSON).bodyValue(body).retrieve().onStatus(status -> status.value() == 401 || status.value() == 402, response -> response.bodyToMono(String.class).flatMap(err -> {
                ...
                }).doOnSubscribe(s -> log.info("🔔 TTS_POST text=\"{}...\"", text.substring(0, Math.min(20, text.length())))).doOnSuccess(chunk -> {
                    if (chunk != null) {
