...
import com.doova.ktab.interactivestorytelling.dto.GeneratedTurn;
import com.doova.ktab.interactivestorytelling.dto.MemorySummary;
import com.doova.ktab.interactivestorytelling.enums.ChoiceId;
import com.doova.ktab.interactivestorytelling.enums.SessionStatus;
import com.doova.ktab.interactivestorytelling.image.*;
import com.doova.ktab.interactivestorytelling.model.*;
import com.doova.ktab.interactivestorytelling.repository.*;
import com.doova.ktab.interactivestorytelling.scd.SceneCanonicalDescription;
import com.doova.ktab.interactivestorytelling.scd.ScdPromptFactory;
...
import com.doova.ktab.service.file.S3Service;
...
public class StorySessionService {
    ...
    private void maybeSummarize(ReadingSession session, Story story) {
        ...
        // JPA dirty checking will UPDATE turns
    }
    ...
    private void generateAndAttachImage(Turn turn, Story story) {
        try {
            ...
            if (turn.getTurnIndex() > 1 && session != null) {
                ...
                if (prev.isPresent()) {
                    ...
                    if (prevAttachment.isPresent()) {
                        prevImageBytes = s3Service.getBytes(prevAttachment.get().getStoragePath());
                    }
                }
            }
            ...
            // Ensure turn has ID (should be set after flush, but double-check)
            Long turnId = turn.getId();
            if (turnId == null) {
                throw new IllegalStateException("Turn ID is null - turn must be saved before generating image");
            }

            String key = s3Service.storeBytes(imageBytes, "image/png", "story-images", ".png");
