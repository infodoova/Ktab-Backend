package com.doova.ktab.ai.gemini.prompt;
...
""";
    ...
    public Prompt buildPrompt(GenerateEndingCommand command) {
        ...
        String rendered = template.render(
                Map.of(
                        "APPROX_WORD_COUNT_FOR_ENDING", command.approxWordCountForEnding(),
                        "TARGET_AUDIENCE_AGE_AND_PROFILE", command.audienceProfile().getProfileText().trim()
                )
        );
        ...
        UserMessage userMessage = UserMessage.builder()
                .text(rendered)
                .media(List.of(pdfMedia)) // or .media(pdfMedia)
                .build();
