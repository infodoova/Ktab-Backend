package com.doova.ktab.ai.prompt;
...
public class ConclusionPromptBuilder {
    ...
    public Prompt build(ConclusionRequest request) throws IOException {
        ...
        String systemPromptText = EndingGeneratorPrompt.build(request.wordCount(), request.audience());

        // 3. Build PDF/File attachment (Media)
        Media fileMedia = Media.builder().mimeType(MimeType.valueOf(contentType)).data(request.file().getBytes()).name(request.file().getOriginalFilename()).build();

        // 4. Create User Message with file attachment
        UserMessage userMessage = UserMessage.builder().text(USER_PROMPT_TEXT).media(fileMedia).build();

        // 5. System + User prompt for the AI model
        return new Prompt(new SystemMessage(systemPromptText), userMessage);
    }
}
